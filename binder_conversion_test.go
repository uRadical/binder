...
func TestSetUintFromEveryKind(t *testing.T) {
	tests := []struct {
		...
		{"json.Number exponent", json.Number("1e3"), 1000, false},
		// "0.0" fails an exact integer parse and reaches the float branch,
		// where zero must be accepted rather than treated as negative.
		{"json.Number zero via float", json.Number("0.0"), 0, false},
